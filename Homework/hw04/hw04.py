...
def balanced(m):
    ...
    "*** YOUR CODE HERE ***"
    def torque(side):
        return total_weight(end(side)) * length(side)

    if is_weight(m):
        return True
    elif is_mobile(m):
        is_this_balanced = torque(left(m)) == torque(right(m))
        return is_this_balanced and balanced(end(left(m))) and balanced(end(right(m)))
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        ...
        new_branches = [replace_leaf(n, old, new) for n in branches(t)]
