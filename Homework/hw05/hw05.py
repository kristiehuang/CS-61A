...
class VendingMachine:
    ...
    def deposit(self, money):
        if self.quantity != 0:
            ...
            string = 'Current balance: ${0}'.format(self.deposited)
        else:
            string = 'Machine is out of stock. Here is your ${0}.'.format(money)
        return(string)
    ...
    def vend(self):
        if self.quantity == 0:
            ...
            change = '' if self.deposited == self.price else ' and ${0} change'.format(self.deposited - self.price)
            self.deposited = 0
            self.quantity -= 1
...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    linked = Link(int(str(n)[0]))
    if n >= 10:
        linked.rest = store_digits(int(str(n)[1:]))
    return linked
